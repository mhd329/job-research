...
- 이 사항들은 [기술](#1-선택한-개발-직군의-종류) 태그를 기준으로 조사를 한 것이기 때문에,

  [기술](#1-선택한-개발-직군의-종류)에 해당하지 않는 언어들은 아래 리스트에 포함되지 않았다는 점 유의하시기 바랍니다.
...
- 필터를 Python 으로 설정하고 공고를 보았음에도 불구하고 Java, Javascript/Typescript, React 등이 많이 보인다.
- 서브 언어로 어느정도 병행하며 공부할 필요가 있어보인다.
- 특히 자바스크립트나 타입스크립트는 프론트엔드 개발 시 대부분 회사에서 쓰이는 것을 볼 수 있었다.
- Python 은 프론트, 백, 다양한 연구 등 여러 분야가 있겠지만 백엔드와 데이터 분야에서 많이 쓰이고 있었다.
- AWS 기반 서비스를 운영하는 능력은 대부분의 회사에서 지원자에게 공통적으로 요구하고 있었다.    

​     

#### 	< 프론트엔드 지망이라면 >

- 자바스크립트, 리엑트정도를 병행해야 할 것 같다.

#### 	< 백엔드 지망이라면 >

- 기존 멀티캠퍼스 커리큘럼만 따라가도(Django, Python) 무리는 없어 보인다.    
